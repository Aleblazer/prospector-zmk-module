 ...
 * "USB" while the dongle is sending over USB. Always visible; it used to fade
 * out three seconds after switching to USB, which left an empty patch in the
 * flow field. The line segments widget keeps the cells behind it clear.
 */
...
static void update_output_widget(struct zmk_widget_output *widget, uint8_t profile_index) {
    ...
    if (is_connected) {
        ...
        lv_obj_set_style_text_color(widget->profile_label, lv_color_hex(NUM_INACTIVE),
                                    LV_PART_MAIN);
        if (is_open) {
            lv_obj_set_style_text_color(widget->links_label, lv_color_hex(SYM_UNPAIRED),
                                        LV_PART_MAIN);
        } else {
            lv_obj_set_style_text_color(widget->links_label, lv_color_hex(SYM_SEARCHING),
                                        LV_PART_MAIN);
